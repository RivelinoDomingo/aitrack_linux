...
#include <locale>
#include <codecvt>
#include </opt/onnxruntime/include/onnxruntime_cxx_api.h> // Certifique-se de incluir o cabeçalho correto.
#include <stdexcept>


// Converta o caminho do modelo para std::string
std::wstring model_path_w = L"model_path.onnx";
std::wstring_convert<std::codecvt_utf8<wchar_t>> converter;
std::string model_path = converter.to_bytes(model_path_w);

// Inicializar o ambiente do ONNX Runtime
Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "StandardTracker");

// Configurar as opções de sessão
// Ort::SessionOptions session_options;
// OrtSessionOptions.SetIntraOpNumThreads(1); // Configure conforme necessário.
...
StandardTracker::StandardTracker(std::unique_ptr<PositionSolver>&& solver, std::wstring& model_path, std::wstring& landmark_model_path)
    : buffer_data(150528), // Inicializa o tamanho do vetor
      solver(std::move(solver)),
      env(ORT_LOGGING_LEVEL_WARNING, "StandardTracker"),
      session_options()
{
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    session_options.SetIntraOpNumThreads(1);
    ...
    std::wstring_convert<std::codecvt_utf8<wchar_t>> converter;
    std::string model_path_str = converter.to_bytes(model_path);

    // session_lm = std::make_unique<Ort::Session>(env, model_path_str.c_str(), session_options);
    session_fd = std::make_unique<Ort::Session>(env, detection_model_path.c_str(), session_options);
    session_lm = std::make_unique<Ort::Session>(env, landmark_model_path.c_str(), session_options);


    // Inicializar o tensor_input_size
    this->tensor_input_size = get_lm_input_size();
    ...
    face_detector = cv::FaceDetectorYN::create(
        std::string(model_path.begin(), model_path.end()),
        "",
        cv::Size(114, 114),
        score_threshold,
        nms_threshold,
...

{
    ...
    cv::resize(image, resized, cv::Size(114, 114), 0.0, 0.0, cv::INTER_LINEAR);
...
void StandardTracker::detect_landmarks(const cv::Mat& image, int x0, int y0, float scale_x, float scale_y, FaceData& face_data) {
    ...
    improc.normalize_and_transpose(resized, buffer_data); // Normaliza e prepara os dados

    Ort::MemoryInfo memory_info = Ort::MemoryInfo::CreateCpu(OrtDeviceAllocator, OrtMemTypeDefault);

    size_t expected_size = 1;
    for (int i = 0; i < 4; ++i) {
        expected_size *= tensor_input_dims[i];
    }

    if (buffer_data.size() < expected_size) {
        throw std::runtime_error("Tamanho do buffer é insuficiente para criar o tensor.");
    }

    Ort::Value input_tensor = Ort::Value::CreateTensor<float>(
        memory_info, buffer_data.data(), tensor_input_size, tensor_input_dims, 4);

    auto output_tensors = session_lm->Run(
        Ort::RunOptions{ nullptr },
        landmarks_input_node_names.data(),
        &input_tensor, 1,
        landmarks_output_node_names.data(),
        1);
